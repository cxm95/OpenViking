...
from openviking.service.core import _resolve_queuefs_mount_point
...
def test_resolve_queuefs_mount_point_defaults_to_shared():
    ...
    assert _resolve_queuefs_mount_point(config) == "/queue"


def test_resolve_queuefs_mount_point_worker_mode_uses_process_index(monkeypatch):
    monkeypatch.setattr(
        "openviking.service.core.multiprocessing.current_process",
        lambda: SimpleNamespace(_identity=(3,)),
    )
    config = StorageConfig(agfs={"queuefs": {"mode": "worker"}})

    assert _resolve_queuefs_mount_point(config) == "/queue/worker-2"
...
def test_resolve_queuefs_mount_point_worker_mode_falls_back_to_pid(monkeypatch):
    ...
    assert _resolve_queuefs_mount_point(config) == "/queue/worker-43210"
