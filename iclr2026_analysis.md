...
本报告分析了 40 篇 ICLR 2026 论文，涵盖 LLM Agent 记忆、持续学习、长上下文处理、知识推理等核心议题。其中 Paper 1（AgentFlow）和 Paper 2（MemAgent）为 Oral 论文，置于最前。

## 一、逐篇论文介绍

## 1. AgentFlow: In-the-Flow Agentic System Optimization for Effective Planning and Tool Use

- **作者**：Zhuofeng Li, Haoxiang Zhang, Seungju Han, Sheng Liu, Jianwen Xie et al.
- **关键词**：Reinforcement Learning, Large Language Models, Agentic Systems, Tool Use, Planning, On-policy Optimization, Sparse Rewards
- **链接**：https://openreview.net/forum?id=Mf5AleTUVK

问题与动机：结果驱动的强化学习推动了 LLM 推理，但现有工具增强方法训练单一单体策略，在完整上下文中交织思考和工具调用——这随长视野和多样工具扩展性差，且对新场景泛化弱。Agent 系统通过将工作分解到专门模块提供了有前景的替代方案，但大多数仍是训练无关的或依赖与多轮交互动态脱节的离线训练。

方法：提出 AgentFlow，可训练的"流中"（in-the-flow）Agent 框架，通过不断演化的记忆协调四个模块（规划器、执行器、验证器、生成器），并直接在多轮循环内优化规划器。提出 Flow-GRPO（Flow-based Group Refined Policy Optimization），通过将多轮优化转化为一系列可处理的单轮策略更新来解决长视野稀疏奖励信用分配问题：将单一可验证的轨迹级结果广播到每一轮，使局部规划器决策与全局成功对齐，并通过组归一化优势稳定学习。

结果：7B 骨干的 AgentFlow 在 10 个基准上超越顶级基线——搜索任务 +14.9%、Agent 任务 +14.0%、数学任务 +14.5%、科学任务 +4.1%，甚至超越 GPT-4o 等更大私有模型。分析确认流中优化的收益：改进规划、增强工具调用可靠性、随模型规模和推理轮数正向扩展。

## 2. MemAgent: Reshaping Long-Context LLM with Multi-Conv RL-based Memory Agent

- **作者**：Hongli Yu, Tinghong Chen, Jiangtao Feng, Jiangjie Chen, Weinan Dai et al.
- **关键词**：LLM, memory, agent, RLVR
- **链接**：https://openreview.net/forum?id=k5nIOvYGCL

问题与动机：尽管长度外推、高效注意力和记忆模块带来了改进，但在外推过程中无性能退化地处理无限长文档仍是长文本处理的终极挑战。

方法：提出 MemAgent，一种新颖的 Agent 工作流——分段处理文本，通过覆盖策略更新记忆，通过增强的记忆管理解决长上下文任务。进一步将 DAPO 算法扩展为直接以端到端方式优化记忆能力，通过独立上下文的多对话生成促进训练。

结果：MemAgent 展现出卓越的长上下文能力——能从 8K 上下文外推到 3.5M QA 任务，性能损失不到 10%，在 512K NIAH 测试上达到 95% 以上。
...
## 4. Pretraining with Hierarchical Memories: Separating Long-Tail and Common Knowledge
...
## 5. CoMem: Compositional Concept-Graph Memory for Vision-Language Adaptation
...
## 6. Look Back to Reason Forward: Revisitable Memory for Long-Context LLM Agents (ReMemR1)
...
## 7. AssoMem: Scalable Memory QA with Multi-Signal Associative Retrieval
...
## 8. HALO: A Memory-Efficient Hierarchical Algorithm for Large-scale Optimal Transport Problems
...
## 9. Memory-Statistics Tradeoff in Continual Learning with Structural Regularization
...
## 10. RF-Mem: Evoking User Memory — Personalizing LLM via Recollection-Familiarity Adaptive Retrieval
...
## 11. BrowseNet: Graph-Based Associative Memory for Contextual Information Retrieval
...
## 12. GraphPlanner: Graph Memory-Augmented Agentic Routing for Multi-Agent LLMs
...
## 13. M3-Agent: A Multimodal Agent with Long-Term Memory
...
## 14. EMPO²: Exploratory Memory-Augmented LLM Agent via Hybrid On- and Off-Policy Optimization
...
## 15. MemGAS: From Single to Multi-Granularity — Toward Long-Term Memory Association and Selection of Conversational Agents
...
## 16. TokMem: One-Token Procedural Memory for Large Language Models
...
## 17. Forget Forgetting: Continual Learning in a World of Abundant Memory
...
## 18. FlowSearcher: Synthesizing Memory-Guided Agentic Workflows for Web Information Seeking
...
## 19. Multi-Agent Debate with Memory Masking (MAD-M²)
...
## 20. GLoW: Dual-Scale World Memory for LLM Agents towards Hard-Exploration Problems
...
## 21. REM: Redirection for Erasing Memory — Towards a Universal Unlearning Method for Corrupted Data
...
## 22. Memento: Toward an All-Day Proactive Assistant for Ultra-Long Streaming Video
...
## 23. Sculptor: Empowering LLMs with Cognitive Agency via Active Context Management
...
## 24. PM-KVQ: Progressive Mixed-precision KV Cache Quantization for Long-CoT LLMs
...
## 25. TNT: Improving Chunkwise Training for Test-Time Memorization
...
## 26. Learning Facts at Scale with Active Reading
...
## 27. When Agents "Misremember" Collectively: Exploring the Mandela Effect in LLM-based Multi-Agent Systems
...
## 28. XENON: Experience-based Knowledge Correction for Robust Planning in Minecraft
...
## 29. Graph-based Nearest Neighbors with Dynamic Updates via Random Walks
...
## 30. Learning From the Past with Cascading Eligibility Traces
...
## 31. ACE: Agentic Context Engineering for Self-Improving Language Models
...
## 32. KRLM: Knowledge Reasoning Language Model — Unifying Knowledge and Language for Inductive Knowledge Graph Reasoning
...
## 33. DataMind: Scaling Generalist Data-Analytic Agents
...
## 34. Test-Time Training Done Right (LaCT)
...
## 35. MemER: Scaling up Memory for Robotic Control via Experience Retrieval
...
## 36. LightMem: Lightweight and Efficient Memory-Augmented Generation
...
## 37. Improving Code Localization with Repository Memory
...
## 38. MemGen: Weaving Generative Latent Memory for Self-Evolving Agents
...
## 39. ReasoningBank: Scaling Agent Self-Evolving with Reasoning Memory
...
## 40. MemoryAgentBench: Evaluating Memory in LLM Agents via Incremental Multi-Turn Interactions
...
40 篇论文中，**超过 32 篇直接涉及"记忆"**，覆盖了从理论到应用、从模型架构到 Agent 系统的完整谱系。这反映了一个明确的趋势：大模型研究正从"如何让模型更大"转向"如何让模型记住"——记忆是实现持续学习、个性化、长期交互和自主进化的关键瓶颈。

## 2. 六大技术方向

| 方向 | 代表论文 | 核心问题 |
|------|----------|----------|
| **Agent 记忆** | REMem, ReMemR1, MemGAS, EMPO², MemGen, ReasoningBank, AgentFlow | Agent 如何存储、检索和利用交互经验 |
...
| **长上下文效率** | PM-KVQ, Memento, Sculptor, TNT, LaCT, MemAgent | 如何在超长上下文下保持效率 |
...
早期记忆系统（如 RAG）主要关注存储和检索，而 2026 年的论文普遍强调记忆与推理的深度耦合。REMem 的情景推理、ReMemR1 的回调增强推理、ReasoningBank 的推理策略提炼、MemGen 的推理-记忆交织、AgentFlow 的流中规划优化——都表明记忆不再是被动仓库，而是推理过程的有机组成。特别是 AgentFlow（Oral）展示了在多轮交互循环中直接优化规划器的可行性，MemGen 展示了推理和记忆的紧密交织循环，ReasoningBank 证明了从推理经验中提炼可泛化策略的价值。
...
Sculptor 的主动上下文管理、ACE 的上下文工程、Memento 的前瞻式感知、MemAgent 的分段覆盖策略——这些工作强调 Agent 应该主动决定"记住什么、遗忘什么"，而非被动地存储所有信息。Sculptor 让 LLM 使用工具主动雕塑内部工作记忆（分片、摘要/隐藏/恢复、精确搜索），ACE 将上下文视为可演化的"剧本"而非静态指令，MemAgent（Oral）通过分段处理+覆盖更新实现从 8K 到 3.5M 的无损外推。这与人类认知的注意力和选择性编码高度一致。
...
| 长文本处理 | MemAgent | 8K→3.5M 无损外推，NIAH 95%+ |
| 多工具规划 | AgentFlow | 流中优化四模块，7B 超越 GPT-4o |
| 个性化对话 | RF-Mem, MemGAS | 双过程检索/多粒度关联 |
| 知识图谱推理 | KRLM | 统一 LLM 知识与 KG 上下文 |

## 6. 待解决的关键挑战

1. **记忆的可扩展性**：随着交互历史增长，记忆检索的准确性和效率如何保持？AssoMem 和 LightMem 分别从多信号融合和三阶段压缩角度做了尝试，MemAgent（Oral）展示了从 8K 到 3.5M 的外推能力，但更长尺度下的表现仍是开放问题。
...
ICLR 2026 的这 40 篇论文共同描绘了一幅图景：**大模型的下一个核心挑战是记忆**。这不仅是技术问题——如何高效存储和检索——更是认知问题——如何像人类一样在时间中积累经验、修正知识、并在此基础上推理和行动。从情景记忆到过程记忆，从图结构到潜在空间，从被动检索到主动管理，这些工作正在构建一个日益完整的 AI 记忆体系。

四个特别值得关注的信号：(1) **MemAgent（Oral）的极限外推**——从 8K 上下文外推到 3.5M QA 任务性能损失不到 10%，展示了记忆管理解决超长上下文的实际可行性；(2) **AgentFlow（Oral）的流中优化**——直接在多轮交互循环内优化规划器，7B 模型超越 GPT-4o，证明了 Agent 系统可训练化的巨大潜力；(3) **MemGen 的自发涌现**——无需显式监督即涌现出规划/过程/工作记忆三类功能，暗示正确的记忆架构可能自然催生类人认知结构；(4) **MemoryAgentBench 的四能力缺失发现**——现有方法无法同时掌握准确检索、测试时学习、长程理解和选择性遗忘，为未来研究划定了明确目标。
